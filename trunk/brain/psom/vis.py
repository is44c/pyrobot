...
class VisPsom(psom):
   ...
   def __init__(self, *args, **keys):
      ...
      self.canvas = Canvas(self.win,
                           width=self.xdim * (2*self.vis_radius+2*self.vis_padding) + self.vis_radius+self.vis_padding,
      ...
      #for now, assume hexagonal
      for y in range(self.ydim):
         self.cells.append([])
         for x in range(self.xdim):
            x0 = (cellwidth * x) + self.vis_padding + (cellwidth/2 * (y % 2))
