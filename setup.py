# Usage:
# * For pyro developers (needs swig):
#       python setup.py build
# * For pyro users (does not need swig):
...
#    - add the <new_device> sources (*.c,*.cpp) in the pyro.camera.device._devices
...
try:
    ...
    print >> f, "def version(): return %r" % PYRO_VERSION
    f.close()
except:
...
setup(
    ...
    # extension modules
    ext_modules = [
        AutoIncludeExtension("pyro.brain.psom._csom",
        ...
        AutoIncludeExtension("pyro.vision.cvision._vision",
            ["vision/cvision/%s" % f for f in "Vision.i", "Vision.cpp"],
            include_dirs = ["camera/device"]),

        AutoIncludeExtension("pyro.vision.example._myvision",
            ["vision/example/%s" % f for f in "myVision.i", "myVision.cpp"],
            include_dirs = ["camera/device"]),

        AutoIncludeExtension("pyro.camera.device._devices",
            ["camera/%s" % f for f in
