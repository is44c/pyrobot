...
def randomArray(size, max):
    temp = RandomArray.random(size) * max * 2.0 - max
    return temp - (max / 2.0)
...
class Network:
    ...
    def loadInput(self, pos, start = 0):
        ...
        if self.inputMapCount == 0:
            ...
            for vals in self.inputMap:
                (v1, offset) = vals
                self.copyVector(self.getLayer(v1).activation, self.input[pos], offset)
    ...
    def loadTarget(self, pos, start = 0):
        ...
        if self.outputMapCount == 0:
            self.copyVector(self.layer[self.layerCount - 1].target, self.output[pos], start)
        else: # mapOutput set manually
            for vals in self.outputMap:
                (v1, offset) = vals
                self.copyVector(self.getLayer(v1).target, self.input[pos], offset)
    ...
    def train(self):
        ...
        #totalPatterns = len(self.output) * len(self.input[0]) / self.layer[0].size
        self.resetCount = 0
        while totalCount != 0 and totalCorrect * 1.0 / totalCount < self.stopPercent:
    ...
    def preprop(self, pattern, sequence):
        self.loadInput(pattern, sequence)
        self.loadTarget(pattern)
    ...
    def addThreeLayers(self, inc, hidc, outc):
        n.add( Layer('input', inc) )
        n.add( Layer('hidden', hidc) )
        n.add( Layer('output', outc) )
    ...
    def saveNetworkToFile(self, filename, saveData = 1):
        ...
        fp.writelines("from pyro.emergent.conx import *\n\n")
...
class SRN(Network):
    def preprop(self, patnum, seq):
        Network.preprop(self, patnum, seq)
        if seq == 0:
            self.getLayer('context').activation = Numeric.ones(self.getLayer('context').size, 'f') * .5

    def postprop(self, patnum, seq):
        Network.postprop(self, patnum, seq)
        if ((seq + 1) < self.sequenceLength):
...
if __name__ == '__main__':
    ...
    # from pyro.emergent.conx import *
