...
PyObject *Aibo::updateMMap(PyObject *points, PyObject *lines) {
  PyObject *pylist;
  int r, g, b;
  int i, x, y;
  char *name;
...
Aibo::Aibo(int wi, int he, int de) {
  initialize(wi, he, de, 0, 1, 2);
  clear();
}

void Aibo::setPixel(int x, int y, int r, int g, int b) {
  image[(y * width + x) * depth + rgb[0]] = r;
  image[(y * width + x) * depth + rgb[1]] = g;
  image[(y * width + x) * depth + rgb[2]] = b;
}

void Aibo::drawLine(long int x0, long int y0, 
...
void Aibo::drawRect(long int x1, long int y1, 
