...
class AiboCamera(Camera):
   ...
   def __init__(self, robot, width=80, height=60, depth = 3, visionSystem = None):
      ...
      self.height = height
      self.depth = depth
      self.cameraDevice = Aibo(self.width, self.height,
                                  self.depth)
      ...
      # from page 31 of the Aibo Soccer Simulator manual:
   ...
   def _update(self):
      try:
         ...
         print "waiting for Aibo camera to come online..."
      ...
      try:
         ...
         print "error in AiboCamera data format: ignored"
